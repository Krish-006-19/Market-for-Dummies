...
export default function StockInfo() {
  ...
  // const [showSipControls, setShowSipControls] = useState(false);
  // const [sipMode, setSipMode] = useState("sip"); // "sip" | "sell"
  ...
  const handlePrimaryAction = async () => {
    ...
    try {
      // Adjust this payload if your backend expects a different stop action.
      const res = await axios.patch(
        `${API_BASE_URL}/portfolio/${symbol}`,
        {
          type: "BUY",
  ...
  //   isBusy ||
  //   (sipMode === "sell"
  //     ? !sellUnits || Number(sellUnits) <= 0 || (!hasUnits && !sipActive)
